...
| **Device / CIBA(Client-Initiated Backchannel Authentication)** | Device code polling; backchannel AuthN for known users | [§1e](01E-device-authorization-and-ciba.md) |
...
| **Denylist Redis keys** | Basic/advanced Redis key shapes, TTL, user epoch, logout-all index | [§3c](03C-denylist-redis-patterns.md) |
...
| Treating access token and ID token as interchangeable | ID token = identity to the client; access token = AuthZ to APIs — [§2](02-oidc-discovery-and-tokens.md) |
